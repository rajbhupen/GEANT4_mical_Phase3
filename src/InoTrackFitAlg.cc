...
// NO rand nos. in MS
//InitTrack posi correction
// Getpropagator using small b : jim
//Derivative 0 by hand
//using getfitdata_new to store _kalman branches.. also added inposX branches
//updated getfitdata to store _kalman branches.. also added inposX branches
/*
...
InoTrackFitAlg::InoTrackFitAlg() {
  ...
  //	
  //  ZPosLayer[11] = (1/m)*(paradef->GetParroom(2));//coordinates on roof
  ZPosLayer[10] = ZPosLayer[9]+0.101;
  ZPosLayer[11] = (1/m)*(paradef->GetINOroomPos(2)+paradef->GetParairroom(2)-1*mm);//room top
...
void InoTrackFitAlg::RunTheFitter( ) {
  if(debug_fit) { cout<< " InoTrackFitAlg::RunTheFitter()" <<PassTrack<<endl;}
  ...
  if(ZIncreasesWithTime==true) {
    ...
    if(ClusterFound==true) {
      ...
      if(LastIteration){
	cout<<"extrapolating the track to Dummy layer(Not in gdml) for CMVD"<<endl;
			...
			dsdzvx = sqrt(1.0 + xvtx_para_roof[2]*xvtx_para_roof[2] + xvtx_para_roof[3]*xvtx_para_roof[3]);
			xvtx_para4roof[1] = acos(-1./dsdzvx);
...
void InoTrackFitAlg::RemoveTrkHitsInShw() {
  ...
  if(debug_fit)  { cout <<"InoTrackFitAlg : RemoveTrkHitsInShw, Discard track finding data in shower" << endl;}
...
void InoTrackFitAlg::GetFitData(int& Plane1, int& Plane2) {
  ...
  if(debug_fit) { cout <<"InoTrackFitAlg : GetFitData "<<"Plane1 "<<Plane1<<" Plane2 "<<Plane2<<endl;}
  ...
  for (unsigned ijk=0; ijk<doubleLa; ijk++) {
    ...
    for (unsigned ij=0; ij<FilteredData[ijk].size(); ij++) {
      ...
      for(unsigned int kl=0; kl<SlcClustData[jk].size(); ++kl) {
       ...
       if(SlcClustData[jk][kl].csh->HitsInCluster.size()>4) continue;

        // if(SlcClustData[ij][jk].csh->GetXEntries()>3) continue;                                                 
        // if(SlcClustData[ij][jk].csh->GetYEntries()>3) continue;                                                 
        if(SlcClustData[jk][kl].csh->GetNXStripsInClust()>3) continue;
        if(SlcClustData[jk][kl].csh->GetNYStripsInClust()>3) continue;
    ...
    if(LastIteration==true) {
                                  pAnalysis->hxpos_ext_kalman[SlcClustData[jk][kl].csh->GetZPlane()]->Fill((SlcClustData[jk][kl].csh->GetXPos()-x1)/StripXWidth);
                                  pAnalysis->hypos_ext_kalman[SlcClustData[jk][kl].csh->GetZPlane()]->Fill((SlcClustData[jk][kl].csh->GetYPos()-y1)/StripYWidth);
                                }
    ...
    if(LastIteration==true) {
                                  pAnalysis->inPosX[SlcClustData[jk][klx].csh->GetZPlane()] = SlcClustData[jk][klx].csh->GetXPos();
                                  pAnalysis->extPosX[SlcClustData[jk][klx].csh->GetZPlane()] = x1;
                                  pAnalysis->inPosY[SlcClustData[jk][klx].csh->GetZPlane()] = SlcClustData[jk][klx].csh->GetYPos();
                                  pAnalysis->extPosY[SlcClustData[jk][klx].csh->GetZPlane()] = y1;
                                }
...
void InoTrackFitAlg::SetTrackProperties(double* x_xk, double* y_yk, double* z_zk) {
  ...
  cout<<"Vtx Parameter from InoTrackFitAlg "<<y_yk[kk]<<endl;
  }
  fTrackCand->SetExtPara(y_yk);
  //RSA roof
  for(int ki=0;ki<6;ki++){
  cout<<"Roof Parameter from InoTrackFitAlg "<<z_zk[ki]<<endl;
  }
...
bool InoTrackFitAlg::PredictedStateCov(double *StateVector, const int Plane, int &NewPlane, const bool GoForward, double* x__minus, int isHalf, double* dS, double* Range) { //, double* dE) {
  ...
  double epsilon=1.e-6;					//1micron shift in extrapolation
    if(NewPlane==11 || NewPlane==10 ){DiffPlane=1;} //30062022 RSA for roof
  //  double GPos[3]={0.0};
  double UxPos[3]={0.0};
  double UyPos[3]={0.0};
  double UxxPos[3]={0.0};
 ...
 if(NewPlane==11 || NewPlane==10){totdist =ZPosLayer[NewPlane]-ZPosLayer[Plane]; } //30062022 RSA 
	...
	cout<<"posTrack["<<jk <<"]"<<posTrack[jk]<<" "<<"dirTrack["<<jk <<"]" <<dirTrack[jk]<<endl;
      }


      
      // RSA:  gGeoManager->InitTrack(posTrack, dirTrack);
      // Earlier this was here.. giving wrong results. since this and ELEctroMagneticField function are not commutative.bcoz inside Electromag function init track is again called and this changes the node position
      Bx = By = 0.0;
      ...
      By *= 1000;
        ...
        gGeoManager->InitTrack(posTrack, dirTrack);
      
	 localmat= gGeoManager->GetCurrentVolume()->GetMaterial();
	icalGeometry->FindNextBoundary();
	cout<<"gGeoManager->GetStep()"<<gGeoManager->GetStep()<<endl;
	...
	Z		= localmat->GetZ();
      ...
      cout<<"Current Vol Name: "<<gGeoManager->GetCurrentVolume()->GetName()<<endl;
      
      snext	= epsilon+gGeoManager->GetStep(); // 1micron is fine since our least thick material is of 50 micrn
      ////	localmat= gGeoManager->GetCurrentVolume()->GetMaterial();
      //  //    Z		= localmat->GetZ();
	////      cout<<" material "<<localmat->GetName()<<"     Z "<<Z<<"     A "<<localmat->GetA()<<endl;
    ...
    // dxdz = i2state[2];
    // dydz = i2state[3];
    // dsdz = pow((1.+pow(i1state[2],2)+pow(i1state[3],2)),0.5);
    ...
    cout<<"..Printing C_k_intermediate.."<<endl;
    for (int jk = 0; jk < 5; jk++)	{
      for (int kl = 0; kl < 5; kl++) {
        C_k_intermediate[jk][kl] = 0;
        C_k_intermediate[jk][kl] = C_k_minus[jk][kl];
	cout<<C_k_intermediate[jk][kl]<<endl;
      }
    }
  }// for loop ends here
  //cout<<dzFe<<"     "<<dznF<<endl;
...
*/
bool InoTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){
  cout<<"bool InoTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){"<<endl;
  //	G4double Dist;
  //	G4double a, b;
  bool ok;
