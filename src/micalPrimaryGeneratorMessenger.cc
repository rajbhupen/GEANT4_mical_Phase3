...
:micalAction(micalGun)
{
  ...
  FluxFileDirCmd = new G4UIcmdWithAString("/mical/gun/flux_file_dir",this);
  FluxFileDirCmd->SetGuidance(" Flux directory name");
  FluxFileDirCmd->SetParameterName("flux_dir",true, true);
  FluxFileDirCmd->SetDefaultValue("./");
...
micalPrimaryGeneratorMessenger::~micalPrimaryGeneratorMessenger() {
  ...
  if (FluxFileDirCmd){delete FluxFileDirCmd;}
...
void micalPrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValue) { 
  ...
  if( command == CorsikaFileNameCmd )
   ...
   if( command == FluxFileDirCmd )
    { micalAction->SetFluxFileDir(newValue);}
